...
export class MdasService {
  ...
  async findAll() {
    ...
    return mdas.map((mda) => {
      ...
      const stalledCount = mda.projects.filter(
        (p) => p.status === 'Stalled',
      ).length;
      const inProgressCount = mda.projects.filter(
        (p) => p.status === 'Ongoing',
      ).length;
      const yetToBeginCount = mda.projects.filter(
        (p) => p.status === 'Not Started',
      ).length;
      const completedCount = mda.projects.filter(
